...
export default function LandingPage(props) {
  ...
  const handleLogout = async () => {
    await fetch("/api/user/logout"); // Perform logout request
      ...
      <nav className="nav-mobile lg:hidden">
        ...
        >
          <div className="flex">
            ...
            >
              {isMobile ? (
                <Image src={Menu} className="h-10 w-10 p-2" alt="Menu Icon" />
              ) : (
                <Image src={Close} className="h-10 w-10 p-3" alt="Close Icon" />
              )}
            </Button>
          </div>
          <div
